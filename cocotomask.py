...
def create_mask(image_info, annotations, output_folder):
    ...
    path_ = output_folder[2] + '/' + image_info['file_name']
    img = cv2.imread(path_)
    mask_path  = output_folder[0] + '/' + image_info['file_name'].split('.jpg')[0] + '.png'
    image_path = output_folder[1] + '/' + image_info['file_name'].split('.jpg')[0] + '.png'
...
def main(json_file, mask_output_folder, image_output_folder, original_image_dir):
    ...
    paths_ = [mask_output_folder, image_output_folder, original_image_dir]
    
    for img in images:
        # Create the masks
        create_mask(img, annotations, paths_)
...
if __name__ == '__main__':
    ...
    #     t2 = f.readlines()    
    original_image_dir = '/home/zafar/old_pc/data_sets/robot-project-datasets/normal-pin-data/factry_bldg/Top_Glass_Abrasion/test/Abrasion-Scratch/train'  # Where your original images are stored
